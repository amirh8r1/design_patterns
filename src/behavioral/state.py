"""
    State
        -a behavioral design pattern that lets an object alter its behavior
...
class Elevator:  # Context
    ...
    def show_state(self):
        print(f'Elevator is in {self.__class__.__name__}')
...
class FirstFloor(Floor):  # Conceret State
    def push_down_btn(self):
...
class SecondFloor(Floor):  # Conceret State
    def push_down_btn(self):
